...
class _DiaryWritePageState extends State<DiaryWritePage> {
  ...
  Future<void> _onSaveTodayDiary() async {
    ...
    await DiaryService().addDiary(diaryEntry);
    for (final e in selected) {
      await LocDiaryService().deleteLocDiary(e.id);
    }
    setState(() {
      _selectedIds.clear();
    });
              ...
              children: [
                ...
                Expanded(
                  child: ListView.builder(
                    itemCount: _locDiaries.length,
                    itemBuilder: (context, index) {
                      final entry = _locDiaries[index];
                      return Card(
                        margin:
                        ...
                        child: ListTile(
                          ...
                          subtitle: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('위치: ${entry.location.displayName}',
                                  style: TextStyle(fontSize: 12)),
                              Text(
                                  '작성: ${entry.createdAt.hour}:${entry.createdAt.minute.toString().padLeft(2, '0')}',
                                  style: TextStyle(fontSize: 12)),
                            ],
                          ),
                        ),
